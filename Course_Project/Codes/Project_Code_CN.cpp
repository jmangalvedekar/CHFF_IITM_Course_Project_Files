...
void init_d(int p, float** mat)
{
	...
	mat[0][0]   = 1.0+6.0*Fo/2.0;
	mat[0][p-1] = 1.0+6.0*Fo/2.0;
	mat[1][0]   = 1.0+5.0*Fo/2.0;
	mat[1][p-1] = 1.0+5.0*Fo/2.0;
	
	for(int i=1;i<p-1;i++)
	{
		mat[0][i] = 1.0+5.0*Fo/2.0;
		mat[1][i] = 1.0+4.0*Fo/2.0;
	}
}

void init_b(int p, float** mat_T, float** T_old_t, float** mat_b, float** q)
{
	mat_b[0][0] 	= 2.0*Fo*(Ty1+Tx1)+(Fo/2.0)*(mat_T[1][0]+T_old_t[1][0])+q[0][0]*dt/(rho*Cp)+(1.0-6.0*Fo/2.0)*T_old_t[0][0];
	mat_b[0][p-1] 	= 2.0*Fo*(Ty2+Tx1)+(Fo/2.0)*(mat_T[1][p-1]+T_old_t[1][p-1])+q[0][p-1]*dt/(rho*Cp)+(1.0-6.0*Fo/2.0)*T_old_t[0][p-1];
	mat_b[p-1][0] 	= 2.0*Fo*(Ty1+Tx2)+(Fo/2.0)*(mat_T[p-2][0]+T_old_t[p-2][0])+q[p-1][0]*dt/(rho*Cp)+(1.0-6.0*Fo/2.0)*T_old_t[p-1][0];
	mat_b[p-1][p-1] = 2.0*Fo*(Ty2+Tx2)+(Fo/2.0)*(mat_T[p-2][p-1]+T_old_t[p-2][p-1])+q[p-1][p-1]*dt/(rho*Cp)+(1.0-6.0*Fo/2.0)*T_old_t[p-1][p-1];
	
	for(int j=1;j<p-1;j++)
	{
		mat_b[0][j]		= 2.0*Fo*Tx1+(Fo/2.0)*(mat_T[1][j]+T_old_t[1][j])+q[0][j]*dt/(rho*Cp)+(1.0-5.0*Fo/2.0)*T_old_t[0][j];
		mat_b[p-1][j]	= 2.0*Fo*Tx2+(Fo/2.0)*(mat_T[p-2][j]+T_old_t[p-2][j])+q[p-1][j]*dt/(rho*Cp)+(1.0-5.0*Fo/2.0)*T_old_t[p-1][j];
	}
	for(int i=1;i<p-1;i++)
	{
		mat_b[i][0] 	= (Fo/2.0)*(mat_T[i-1][0]+mat_T[i+1][0]+T_old_t[i-1][0]+T_old_t[i+1][0])+2.0*Fo*Ty1+q[i][0]*dt/(rho*Cp)+(1.0-5.0*Fo/2.0)*T_old_t[i][0];
		mat_b[i][p-1] 	= (Fo/2.0)*(mat_T[i-1][p-1]+mat_T[i+1][p-1]+T_old_t[i-1][p-1]+T_old_t[i+1][p-1])+2.0*Fo*Ty2+q[i][p-1]*dt/(rho*Cp)+(1.0-5.0*Fo/2.0)*T_old_t[i][p-1];
	}
	...
	for(int i=1;i<p-1;i++)
	{
		for(int j=1; j<p-1;j++)
		{
			mat_b[i][j] = (Fo/2.0)*(mat_T[i-1][j]+mat_T[i+1][j]+T_old_t[i-1][j]+T_old_t[i+1][j])+q[i][j]*dt/(rho*Cp)+(1.0-4.0*Fo/2.0)*T_old_t[i][j];
		}
	}
}
