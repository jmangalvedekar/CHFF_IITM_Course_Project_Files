...
//Defining the Domain
const int n = 3; //No. of elements/nodes
const double length = 3.0; //length of an edge of the square plate
const double gamma = 0.0; //Diffusion coefficient value
...
{	
	...
	for(int i=0;i<m;i++)
	{
		...
		if(residue<0.000001)
		{
    		...
    		cout<<"\nThe phi values across the domain are:\n";
    		print_Temp(n,T);
    		//print_matrix(n,n,T);
    		file.close();
...
{
	double aE=0.0,aW=0.0,aN=0.0,aS=0.0,aP=0.0,Fe=0.0,Fw=0.0,Fn=0.0,Fs=0.0;
	...
	for(int i=1;i<p-1;i++)
	{
		for(int j=1;j<p-1;j++)
		{
			...
			aN = 0.0 + max(-Fn,0.0);
			aS = 0.0 + max(Fs,0.0);
			mat_b[i][j] = -(aW*mat_T[i-1][j]+aE*mat_T[i+1][j])-Sc(i,j)*dv;
			aP = aE+aW+aN+aS+Fe-Fw+Fn-Fs;
			U[i][j] = aN;
			D[i][j] = -aP;
			L[i][j] = aS;
